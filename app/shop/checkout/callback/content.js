...
export default function ShopCallbackContent({ order }) {
      ...
      <div className="min-h-screen bg-[#FAFAFA] flex items-center justify-center p-4 font-brasley-medium">
        <div className="bg-white rounded-2xl shadow-lg p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-600 mb-6">
            We couldn't find your order. Please check your email for order confirmation or contact support.
          </p>
          <Link href="/shop" className="inline-block w-full bg-[#A5914B] text-white font-semibold py-3 px-6 rounded-xl hover:bg-[#8B7A3F] transition-colors">
            Back to Shop
          </Link>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-lg p-8 max-w-md w-full text-center">
        {isSuccess && (
          <>
            ...
            <p className="text-gray-600 mb-6">
              Thank you for your purchase, {order.buyer_firstname || "Customer"}! Your order has been confirmed and is being processed.
            </p>
          </>
        )}
        ...
        <div className="bg-gray-50 rounded-xl p-4 mb-6 text-left">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium text-[#A5914B]">{formatPrice(order.total_amount, order.currency)}</span>
            </div>
            {order.buyer_email && (
              <div className="flex justify-between">
                <span className="text-gray-600">Confirmation sent to</span>
                <span className="font-medium text-gray-900 truncate max-w-[180px]">{order.buyer_email}</span>
              </div>
            )}
          </div>
        </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-blue-700 mt-1">
                  Your order will be shipped via Aramex. You'll receive tracking information via email within 24-48 hours.
                </p>
              </div>
            </div>
          </div>
        ...
        {isFailed && (
          <Link href="/shop" className="inline-block mt-4 text-sm text-[#A5914B] hover:underline">
            Return to shop
          </Link>
        )}
      </div>
