...
export default function ShopCheckoutContent({ userProfile = null }) {
  ...
  const [cartActionState, setCartActionState] = useState({ loading: false, itemId: null, error: null });
  ...
  const [state, formAction, isPending] = useActionState(processShopCheckout, { success: false, error: null });
  ...
  const applyPromoCode = useCallback(async () => {
    ...
    if (!code) {
      setPromoState((prev) => ({ ...prev, applied: false, loading: false, error: "Enter a promo code." }));
      return;
    }
  ...
  useEffect(() => {
    ...
    const loadZones = async () => {
      ...
      try {
        ...
        if (!response.ok) throw new Error(payload?.details || "Failed to load shipping zones.");
        ...
        if (active) setZonesState({ loading: false, error: error?.message || "Failed to load shipping zones." });
      }
    };
    loadZones();
    return () => { active = false; };
  ...
  const destinationAddress = useMemo(() => ({
    ...
    postalCode: "",
    countryCode: "GH",
  }), [formData.address, formData.city, formData.digitalAddress, selectedRegion?.name]);
    ...
    const timeoutId = setTimeout(async () => {
      ...
      try {
        ...
        const payload = await getShopAramexRateQuote({
          origin: { line1: "Accra", line2: "", city: "Accra", state: "Greater Accra", postalCode: "", countryCode: "GH" },
        ...
        if (!payload?.success) throw new Error(payload?.error || "Failed to fetch shipping rate.");
        if (!cancelled) setShippingQuote({ amount: Number(payload?.amount) || null, loading: false, error: null });
      } catch (error) {
        if (!cancelled) setShippingQuote({ amount: null, loading: false, error: error?.message || "Failed to fetch rate." });
      }
    }, 600);
    return () => { cancelled = true; clearTimeout(timeoutId); };
  }, [cartLoading, cartHasItems, destinationAddress, subtotal, totalWeight, totalPieces]);

  // Cart item quantity update
  const updateItemQuantity = useCallback(async (cartItemId, nextQuantity) => {
    ...
    try {
      ...
      setCartActionState({ loading: false, itemId: null, error: error?.message || "Failed to update cart." });
    }
  }, [fetchCartDetails]);
    ...
    return (
      <div className="min-h-screen bg-[#FAFAFA] flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-lg p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-600 mb-6">Your order has been created. You will be redirected to complete payment.</p>
          <a
          ...
          <p className="text-xs text-gray-500 mt-4">Order Reference: {state.orderCode}</p>
        </div>
      </div>
    );
  }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" id="firstName" name="firstName" required value={formData.firstName} onChange={handleInputChange}
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#A5914B]/20 focus:border-[#A5914B] outline-none transition-colors" placeholder="John" />
                </div>
                <div>
                  <label htmlFor="lastName" className="block text-sm font-medium text-gray-700 mb-1">Last Name *</label>
                  <input type="text" id="lastName" name="lastName" required value={formData.lastName} onChange={handleInputChange}
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#A5914B]/20 focus:border-[#A5914B] outline-none transition-colors" placeholder="Doe" />
                </div>
                <div>
                  <label htmlFor="email" className="block text-sm font-medium text-gray-700 mb-1">
                    <Mail className="size-4 inline mr-1" />Email Address *
                  </label>
                  <input type="email" id="email" name="email" required value={formData.email} onChange={handleInputChange}
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#A5914B]/20 focus:border-[#A5914B] outline-none transition-colors" placeholder="john@example.com" />
                </div>
                <div>
                  <label htmlFor="phone" className="block text-sm font-medium text-gray-700 mb-1">
                    <Phone className="size-4 inline mr-1" />Phone Number *
                  </label>
                  <input type="tel" id="phone" name="phone" required value={formData.phone} onChange={handleInputChange}
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#A5914B]/20 focus:border-[#A5914B] outline-none transition-colors" placeholder="0244123456" />
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6 sticky top-6">
              ...
              <div className="border-b border-gray-100 pb-4 space-y-5">
                {cartLoading ? (
                  ...
                  <div className="text-sm text-gray-500 text-center py-4">Your cart is empty.</div>
                ) : (
                  vendorGroups.map((group) => (
                    <div key={group.vendor.slug}>
                      ...
                      <div className="flex items-center gap-2 mb-3">
                        {group.vendor.logo ? (
                          <Image src={group.vendor.logo} alt={group.vendor.name} width={20} height={20} className="rounded-full object-cover" />
                        ) : (
                          <Store className="size-4 text-gray-400" />
                        )}
                        <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide">{group.vendor.name}</span>
                      </div>

                      {/* Items */}
                      <div className="space-y-3">
                        {group.items.map((item) => {
                          const isUpdating = cartActionState.loading && cartActionState.itemId === item.cartItemId;
                          return (
                            <div key={item.cartItemId} className={`flex gap-3 ${isUpdating ? "opacity-50" : ""}`}>
                              <div className="w-14 h-14 rounded-lg overflow-hidden bg-gray-50 border border-gray-200 shrink-0">
                                {item.image ? (
                                  <Image src={item.image} alt={item.name} width={56} height={56} className="object-contain w-full h-full p-0.5" />
                                ) : (
                                  <div className="w-full h-full flex items-center justify-center"><Store className="size-4 text-gray-300" /></div>
                                )}
                              </div>
                              <div className="flex-1 min-w-0">
                                ...
                                {item.variation && (
                                  <p className="text-xs text-gray-500">
                                    {item.variation.label || [item.variation.color, item.variation.size].filter(Boolean).join(" / ")}
                                  </p>
                                )}
                                <div className="flex items-center gap-2 mt-1">
                                  <div className="flex items-center border border-gray-200 rounded-md">
                                    <button type="button" onClick={() => updateItemQuantity(item.cartItemId, (item.quantity || 1) - 1)}
                                    ...
                                    <button type="button" onClick={() => updateItemQuantity(item.cartItemId, (item.quantity || 0) + 1)}
                                      disabled={isUpdating}
                                      className="p-1 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors cursor-pointer">
                                      <Plus className="size-3" />
                                    </button>
                                  </div>
                                  <button type="button" onClick={() => updateItemQuantity(item.cartItemId, 0)} disabled={isUpdating}
                                    className="p-1 text-gray-400 hover:text-red-500 transition-colors cursor-pointer disabled:opacity-50">
                                    <Trash2 className="size-3.5" />
                                  </button>
                                  {isUpdating && <Loader2 className="size-3 animate-spin text-gray-400" />}
                                </div>
                              </div>
                ...
                {cartActionState.error && (
                  <p className="text-xs text-red-600">{cartActionState.error}</p>
                )}
              </div>
              ...
              <div className="py-4 space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">{totalWeight > 0 ? `${totalWeight.toFixed(2)} kg` : "Not set"}</span>
                </div>
                <div className="flex justify-between text-sm">
              ...
              <div className="pb-4 border-b border-gray-100 space-y-2">
                ...
                <div className="flex gap-2">
                  <input type="text" value={promoCode} onChange={(e) => { setPromoCode(e.target.value); if (promoState.applied) resetPromoState(); }}
                  ...
                  <button type="button" onClick={applyPromoCode} disabled={promoState.loading || cartLoading}
                ...
                {promoState.applied ? (
                  <div className="flex items-center justify-between text-xs text-green-700">
                    <span>
                      {promoState.promo?.percent_off || promoState.discount?.percentOff}% off applied to eligible items
                    </span>
                    <button type="button" onClick={resetPromoState} className="text-[#A5914B] hover:underline cursor-pointer">
                      Remove
                    </button>
                  </div>
                ) : promoState.error ? (
              ...
              <input type="hidden" name="promoCode" value={promoState.applied ? promoCode : ""} />

              {/* Submit */}
              <button
                type="submit"
                disabled={isPending || !cartHasItems || cartLoading || cartActionState.loading}
