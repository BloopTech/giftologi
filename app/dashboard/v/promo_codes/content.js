    ...
    try {
      ...
      const [{ data: categoriesData, error: categoriesError }, { data: productsData, error: productsError }] =
        await Promise.all([
          ...
          supabase
            .from("products")
            .select("id, name, vendor_id, status, active")
            .eq("vendor_id", vendorRecord.id)
            .eq("status", "approved")
            .eq("active", true)
            .order("name"),
        ...
        <DialogContent className="max-w-2xl">
          ...
          <form ref={createFormRef} onSubmit={handleCreateSubmit} className="space-y-4">
            ...
            <div className={cx("space-y-4", createStep === 1 ? "" : "hidden")}>
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div className="space-y-1">
                  <label className="text-xs font-medium text-[#0A0A0A]">Start Date</label>
                  <input
                    name="startAt"
                    type="date"
                    min={getTodayString()}
                ...
                <div className="space-y-1">
                  <label className="text-xs font-medium text-[#0A0A0A]">End Date</label>
                  <input
                    name="endAt"
                    type="date"
              ...
              <div className="space-y-3">
                ...
                <div className="max-h-56 overflow-y-auto rounded-lg border border-[#D1D5DB] bg-white p-3 space-y-2">
                  {filteredProducts.length ? (
                    ...
                    <p className="text-[11px] text-[#9CA3AF]">No products match this search.</p>
                  )}
                </div>
              </div>
            </div>
