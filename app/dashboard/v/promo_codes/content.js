...
import { useVendorPromoCodes } from "./context";
...
export default function VendorPromoCodesContent() {
  const {
    vendor,
    promos,
    targetMap,
  ...
  const searchProductsFor = useCallback(async (query, setResults, setLoading, { shippable, categoryIds } = {}) => {
    setLoading(true);
    try {
  ...
  }, [vendor?.id, setSelectedProductsCache]);
  ...
  }, [searchProductsFor, newTargetShippable, newTargetCategoryIds]);
  ...
  }, [searchProductsFor, editTargetShippable, editTargetCategoryIds]);

  useEffect(() => {
    if (createOpen && vendor?.id) searchProductsFor("", setCreateSearchResults, setCreateSearchLoading, {
      shippable: newTargetShippable,
      categoryIds: newTargetCategoryIds,
    });
  }, [createOpen, vendor?.id, searchProductsFor, newTargetShippable, newTargetCategoryIds]);

  useEffect(() => {
    if (editOpen && vendor?.id) searchProductsFor("", setEditSearchResults, setEditSearchLoading, {
      shippable: editTargetShippable,
      categoryIds: editTargetCategoryIds,
    });
  }, [editOpen, vendor?.id, searchProductsFor, editTargetShippable, editTargetCategoryIds]);
  ...
  // newCategorySet removed — CascadingCategoryPicker manages its own set
  const editProductSet = useMemo(
    () => new Set(editTargetProductIds),
    [editTargetProductIds],
  );
  // editCategorySet removed — CascadingCategoryPicker manages its own set
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex flex-col">
          ...
          <span className="text-[#717182] text-xs/4 font-brasley-medium">
            Offer vendor-only discounts for your storefront customers.
          </span>
        </div>
        <button
          type="button"
      ...
      <div className="rounded-2xl border border-[#E5E7EB] bg-white p-4">
        <div className="rounded-xl border border-[#E5E7EB] bg-[#F9FAFB] p-3 text-[11px] text-[#4B5563]">
          Vendor promo codes only apply to your products and are funded by your store. Target specific
          categories or products to run focused campaigns.
        </div>
      </div>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-[#E5E7EB]">
                  <th className="text-left py-2 px-2 font-medium text-[#0A0A0A]">Code</th>
                  <th className="text-left py-2 px-2 font-medium text-[#0A0A0A]">Discount</th>
                  <th className="text-left py-2 px-2 font-medium text-[#0A0A0A]">Targets</th>
                  <th className="text-left py-2 px-2 font-medium text-[#0A0A0A]">Filters</th>
                  <th className="text-left py-2 px-2 font-medium text-[#0A0A0A]">Usage</th>
                  <th className="text-left py-2 px-2 font-medium text-[#0A0A0A]">Schedule</th>
                  <th className="text-left py-2 px-2 font-medium text-[#0A0A0A]">Status</th>
                ...
                {promos.map((promo) => {
                  ...
                  return (
                    <tr key={promo.id} className="border-b border-[#F3F4F6]">
                      ...
                      <td className="py-2 px-2 text-[#0A0A0A]">
                        <div className="flex flex-col">
                          <span className="font-medium">{promo.percent_off}% off</span>
                          <span className="text-[11px] text-[#6B7280]">
                      ...
                      <td className="py-2 px-2 text-[#6B7280]">
                        <span className="text-[11px]">{renderShippableLabel(promo.target_shippable)}</span>
                      </td>
                      <td className="py-2 px-2 text-[#6B7280]">
                        <div className="flex flex-col">
                          <span>
          ...
          <form ref={createFormRef} onSubmit={handleCreateSubmit} className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className={cx(
                      ...
                      focusInput,
                      toErrorList(createState?.errors?.code).length ? hasErrorInput : "",
                    )}
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className={cx(
                      ...
                      "border-[#D6D6D6] text-[#0A0A0A] placeholder:text-[#B0B7C3]",
                      focusInput,
                      toErrorList(createState?.errors?.percentOff).length ? hasErrorInput : "",
                    )}
                    placeholder="20"
                    disabled={createPending}
                  />
            ...
            <div className={cx("space-y-4", createStep === 1 ? "" : "hidden")}>
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                <div className="space-y-1">
                  <label className="text-xs font-medium text-[#0A0A0A]">Minimum Spend</label>
                  <input
                    name="minSpend"
                    type="number"
                    min="0"
                ...
                <div className="space-y-1">
                  <label className="text-xs font-medium text-[#0A0A0A]">Usage Limit</label>
                  <input
                    name="usageLimit"
                    type="number"
                    min="1"
                ...
                <div className="space-y-1">
                  <label className="text-xs font-medium text-[#0A0A0A]">Per User Limit</label>
                  <input
                    name="perUserLimit"
                    type="number"
                    min="1"
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div className="space-y-1">
                  <label className="text-xs font-medium text-[#0A0A0A]">Start Date & Time <span className="text-[11px] font-normal text-[#6B7280]">(GMT)</span></label>
                  <input
                    name="startAt"
                    type="datetime-local"
                    min={getNowUTCString()}
                ...
                <div className="space-y-1">
                  <label className="text-xs font-medium text-[#0A0A0A]">End Date & Time <span className="text-[11px] font-normal text-[#6B7280]">(GMT)</span></label>
                  <input
                    name="endAt"
                    type="datetime-local"
            ...
            <div className={cx("space-y-4", createStep === 2 ? "" : "hidden")}>
              <div className="space-y-3">
                <div>
                  <h3 className="text-xs font-medium text-[#0A0A0A]">Shippability</h3>
                  <p className="text-[11px] text-[#6B7280]">Restrict which products this promo applies to based on shippability.</p>
                </div>
                <div className="flex flex-wrap gap-2">
                  {SHIPPABLE_OPTIONS.map((opt) => (
                    <button
                      ...
                      disabled={createPending}
                      className={cx(
                        "rounded-full px-3 py-1.5 text-[11px] font-medium border transition-colors cursor-pointer",
                  ...
                  {createSearchLoading ? (
                    ...
                    createProductList.map((product) => (
                      ...
                      >
                        ...
                        />
                        <span className="font-medium">{product.name || "Untitled"}</span>
                      </label>
                    ))
                  ) : (
                    <p className="text-[11px] text-[#9CA3AF]">No products found.</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between pt-2">
              <div className="flex items-center gap-2 text-[11px] text-[#6B7280]">
                <span>Step {createStep + 1} of {promoStepLabels.length}</span>
              </div>
              <div className="flex items-center gap-2">
                <DialogClose asChild>
                  <button
                    type="button"
                    className="rounded-full border border-[#E5E7EB] px-4 py-2 text-[11px] font-medium text-[#6B7280] hover:border-primary hover:text-primary cursor-pointer"
                  >
                    Cancel
                  </button>
                </DialogClose>
                {createStep > 0 ? (
                  <button
                    ...
                    className="rounded-full border border-[#E5E7EB] px-4 py-2 text-[11px] font-medium text-[#0A0A0A] hover:border-primary hover:text-primary cursor-pointer"
                    disabled={createPending}
                  >
                    Back
                  </button>
      ...
      >
        <DialogContent className="max-w-2xl">
          ...
          <form ref={editFormRef} onSubmit={handleEditSubmit} className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className={cx(
                      ...
                      "border-[#D6D6D6] text-[#0A0A0A] placeholder:text-[#B0B7C3]",
                      focusInput,
                      toErrorList(updateState?.errors?.code).length ? hasErrorInput : "",
                    )}
                    placeholder="e.g. VENDOR20"
                    disabled={updatePending}
                  />
            ...
            <div className="flex items-center justify-between pt-2">
              <div className="flex items-center gap-2 text-[11px] text-[#6B7280]">
                <span>Step {editStep + 1} of {promoStepLabels.length}</span>
              </div>
              <div className="flex items-center gap-2">
                <DialogClose asChild>
                  <button
